...
async function getPaiementsByClient(req, res) {
    try {
        ...
        res.status(500).json({ message: 'Erreur lors de la récupération des paiements du client.' });
    }
}
...
async function updatePaiementStatus(paiementID, status) {
    try {
        const allowedStatuses = ['En attente', 'Terminé', 'Échoué', 'Remboursé', 'Annulé'];
        if (!allowedStatuses.includes(status)) {
            throw new Error(`Statut de paiement invalide. Statuts valides: ${allowedStatuses.join(', ')}`);
        }
        const paiement = await db('paiement').where({ paiementID }).first();
        if (!paiement) {
        ...
        throw new Error(`Échec de mise à jour du statut du paiement: ${error.message}`);
    }
}
...
const createPaiement = async (req, res) => {
    ...
    if (!factureID || !montant || !paiementDate || !methode || !status) {
        console.error('DEBUG: Missing required fields', { factureID, montant, paiementDate, methode, status });
    ...
    try {
        ...
        if (error.message.includes("Facture introuvable")) {
            return res.status(404).json({ error: "La facture spécifiée n'existe pas." });
        }
        res.status(500).json({
